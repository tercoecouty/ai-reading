...
export default class Parser {
     ...
     * 3. 测量页面的宽度和高度
     */
    measure(dom_span: HTMLElement, dom_page: HTMLElement) {
        this.chineseCharWidth = measureChar("正".charCodeAt(0));
        for (let code of this.charCodeSet) {
            this.charWidthMap.set(code, measureChar(code));
        }
        ...
        function measureChar(code: number): number {
            dom_span.textContent = String.fromCharCode(code);
            return dom_span.getBoundingClientRect().width;
        }
    }
    ...
    decodeItems(items: Uint8Array): IDecodeItem[] {
        ...
        while (true) {
            ...
            if (itemType === EItemType.singleCharBox) {
                ...
                const charCode = this.getUint(items.slice(index, index + 2));
                const shrink = this.getUint(items.slice(index + 2, index + 3)) / 10; // em
                index += 3;
                itemList.push({ itemType, charCodeList: [charCode], shrink });
            } else if (itemType === EItemType.glue) {
                const charCode = this.getUint(items.slice(index, index + 2));
                const stretch = this.getUint(items.slice(index + 2, index + 3)) / 10;
                const shrink = this.getUint(items.slice(index + 3, index + 4)) / 10;
                index += 4;
    ...
    private getUint8List(n: number, bitNumber: number) {
        // n 是 8 的倍数
...
function parseBookText(bookText: string) {
    // todo
}
